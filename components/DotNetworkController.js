class DotNetworkController extends Component {
    ...
    start() {
        // Linebatch to draw all lines (non-batched since each has a different strokeStyle)
    ...
    _updateConnectionLines() {
        ...
        this.dots.forEach(dot => {
            ...
            connections.forEach(neighbor => {
                // Canonical keys to avoid drawing the same connection twice
                ...
                // Apply quiet zone factors to dim appropriately
    ...
    _spawnDots() {
        ...
        for (let i = 0; i < this.config.NUM_DOTS; i++) {
            ...
            dot.transform.setParent(this.transform.parent, false)
    ...
    calcAlphaFactor(x, y) {
        ...
        // Vertical "quiet" zone (based on x position)
