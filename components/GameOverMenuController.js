class GameOverMenuController extends Component {
    ...
    start() {
        ...
        // Title
        const titleGO = GameObject.instantiate(new GameObject("GameOverTitleGO"), {
            scene: this.scene,
            layer: "ui",
        ...
        for (let i = 0; i < this.items.length; i++) {
            const go = GameObject.instantiate(new GameObject(`GameOverItem${i}GO`), {
                scene: this.scene,
                layer: "ui",
    ...
    _makeSelectorLine(side) {
        const go = GameObject.instantiate(new GameObject(`GameOverSelector_${side}`), {
            scene: this.scene,
            layer: "ui",
    ...
    _updateSelectorTargets() {
        ...
        this._leftSel.setTarget({ y, text, font: this.itemFont, centerX: this.centerX })
        this._rightSel.setTarget({ y, text, font: this.itemFont, centerX: this.centerX })
    }

    _activate() {
        if (this.selected === 0) this.gameFlow.playAgain()
